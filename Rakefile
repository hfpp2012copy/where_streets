...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/hfpp2012/where_streets/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("where_streets.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in where_streets.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "where_streets.gemspec", /ruby_version = .*">= (.*)"/ => lowest
